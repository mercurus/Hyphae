...
<template>
    <router-link custom :to="{name:'editMorph', params:{id:record.id}}" v-slot="{ href, navigate }">
        <a @click="navigate" :href="href">
            <div class="media search-result">
                <div class="media-left search-icon-block">
                    <BIcon :icon="record.icon" extraClasses="is-size-2 m-3" />
                    <span class="is-block is-size-7 has-text-weight-bold">{{ record.name }}</span>
                </div>
